...
with open("yelp_academic_dataset_review.json", "r", encoding="latin1") as inputFile:
    slicedFile = islice(inputFile, 675000)
    ...
    # ------creating files and placing tokens in resp. folders------#
    for each_review in reviewRecord:

        stars = each_review['stars']

        if (stars > 3):
            with io.open("positive/file_positive_" + str(file_counter) + ".txt", 'w',
            ...
            with io.open("negative/file_negative_" + str(file_counter) + ".txt", 'w',
            ...
            with io.open("neutral/file_neutral_" + str(file_counter) + ".txt", 'w',
